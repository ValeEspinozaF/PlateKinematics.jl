...
using PlateKinematics: Covariance, CovIsZero
...
function Add_FiniteRotations(FRc1::FiniteRotCart, FRc2::FiniteRotCart, Nsize = 1e5)
    ...
    for i in 1:Nsize
        ...
        FRc[i] = Finrot2Cart(FiniteRotMatrix(FRm2.Values * FRm1.Values))
    end
...
function Add_FiniteRotations(FRs1::FiniteRotSph, FRs2::FiniteRotSph)

    FRc1 = Finrot2Cart(FRs1)
    FRc2 = Finrot2Cart(FRs2)

    # Cartesian vector in [degrees], covariace in [degrees^2]
    FRc = Add_FiniteRotations(FRc1, FRc2)
